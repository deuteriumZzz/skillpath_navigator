...
class SkillPermissionsTestCase(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username='alice', email='alice@ex.com', password='pass12345')
        self.admin = User.objects.create_user(username='admin', email='admin@ex.com', password='pass12345', is_staff=True)
    ...
    def test_regular_user_cannot_create_skill(self):
        self.client.force_authenticate(self.user)
        response = self.client.post('/api/v1/skills/', {'name': 'Go', 'level': 'beginner'})
    ...
    def test_admin_can_create_skill(self):
        ...
        response = self.client.post('/api/v1/skills/', {'name': 'Rust', 'level': 'advanced'})
...
class UserPathPermissionsTestCase(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username='bob', email='bob@ex.com', password='pass12345')
        self.other = User.objects.create_user(username='eve', email='eve@ex.com', password='pass12345')
        self.admin = User.objects.create_user(username='root', email='root@ex.com', password='pass12345', is_staff=True)
...
class PaginationTestCase(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username='carol', email='carol@ex.com', password='pass12345')
        self.client.force_authenticate(self.user)
        for i in range(5):
...
class SkillFilterTestCase(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username='dave', email='dave@ex.com', password='pass12345')
        self.client.force_authenticate(self.user)
