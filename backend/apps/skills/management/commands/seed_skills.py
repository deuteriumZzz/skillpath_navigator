...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--skills', type=Path, default=SKILLS_CSV,
            help='Путь к CSV-файлу с навыками (default: backend/data/skills.csv)',
        )
        parser.add_argument(
            '--deps', type=Path, default=DEPS_CSV,
    ...
    def handle(self, *args, **options):
        ...
        for row in rows:
            ...
            if level not in VALID_LEVELS:
                self.stderr.write(f'  Неверный уровень "{level}" для "{name}", установлен beginner')
                level = 'beginner'
        ...
        with open(deps_path, encoding='utf-8', newline='') as f:
            for row in csv.DictReader(f):
                ...
                try:
                    ...
                    self.stderr.write(f'  Пропущено ({skill_name} -> {depends_on}): {e}')
                    dep_skipped += 1
