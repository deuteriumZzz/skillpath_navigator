...
class ApiEndpointsTestCase(APITestCase):
    ...
    def test_skill_graph(self):
        response = self.client.get('/api/skills/graph/')
        self.assertEqual(response.status_code, 200)
        self.assertIn('nodes', response.data)
        self.assertIn('edges', response.data)

    def test_next_step(self):
        response = self.client.get(f'/api/skills/{self.python.id}/next-step/')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data[0]['skill'], 'Django')

    def test_path_to(self):
        response = self.client.get(f'/api/skills/{self.python.id}/path-to/{self.django.id}/')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['path'], ['Python', 'Django'])

    def test_progress_update(self):
        response = self.client.post('/api/progress/update/', {'skill_id': self.python.id, 'completion_percent': 80})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['completion_percent'], 80)

    def test_learning_path(self):
        response = self.client.post('/api/learning-path/', {'target_skills': ['Django']}, format='json')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['plan'][0]['target'], 'Django')

    def test_user_path(self):
        response = self.client.get(f'/api/users/{self.user.id}/path/')
        self.assertEqual(response.status_code, 200)
